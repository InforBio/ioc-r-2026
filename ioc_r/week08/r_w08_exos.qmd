...
4. Based on the previous figure, add the adjusted p-value using `stat_pvalue_manual()` from the {ggpubr} package. (`?stat_pvalue_manual`)
...
Place `p_lfc` on the left, and arrange the two boxplots on the right (one on top of the other) using the `|` or `/` operators.
