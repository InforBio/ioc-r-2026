...
- When you see the prompt **`+`** (at the beginning of a new line): R is waiting for you to finish your command.
- When you see the `[1]`:  it's an output line
:::
:::
...
1. (R) Console: enter and execute commands interactively.
...
- Open your RStudio.
- Create an R script: *File* -> *New File* -> *R Script* or click ![](images/rstudio_newfile.png){width=1.5cm}. (Shown in 4)
- Write your codes/commandes in the script.
- Save script: *Cmd + S* or *Ctrl + S*, name your script, *e.g.*: `test.R`.
- Execute code: Click ![](images/rstudio_runcode.png){width=1.3cm} or press *Cmd + Enter* (*Ctrl + Enter* for Windows)
...
Importing your first file: [read-counts.csv](https://raw.githubusercontent.com/InforBio/ioc-r-2026/refs/heads/dev-r/ioc_r/exos_data/read-counts.csv)
...
- Do we want to use the 1st row as column names?
...
We'll use the built-in iris dataset and save it in the CSV format using the `write.csv()` function.

```r
head(iris) # show the first lines of the dataset
View(iris) # preview the whole dataset
?write.csv # get help of the function
```
...
```r
write.csv(iris, file = "outputs/iris_export.csv")
```
