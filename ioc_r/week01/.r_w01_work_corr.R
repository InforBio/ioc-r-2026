...
library(readr)
visceral_fat <- read_csv("../exos_data/visceral_fat.csv") # replace the path by your own


# 2. Summarize the dataset.
# Run the function `summary(visceral_fat)` to generate a summary of the dataset.
# Describe what information you obtained.
...
# 3. Draw a histogram.
# Create a histogram for the diastolic blood pressure using the column `visceral_fat[["diastolic_bp__mmhg"]]`.
# Describe what you observe from the plot.

hist(visceral_fat[["diastolic_bp__mmhg"]])

# The histogram shows a skewed distribution with some extreme values.
# Most of the values are concentrated in the bins on the left, while a single bin far on the right suggests outliers.
